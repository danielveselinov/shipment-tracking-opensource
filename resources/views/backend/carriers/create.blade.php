    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('carriers.store') }}" class="p-6 text-gray-900">
                    @csrf
                    <div class="mb-6">
                    ...
                    <div class="mb-6">
                        <input type="text" placeholder="Carrier name" name="name" value="{{ old('name') }}"
                    ...
                    <div class="mb-6">
                        <input type="email" placeholder="Carrier email" name="email" value="{{ old('email') }}"
                               class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 md:w-2/5">
                        @error('email')
                        <p class="text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        <input type="text" placeholder="Carrier phone" name="phone" value="{{ old('phone') }}"
                               class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 md:w-2/5">
                        @error('phone')
                        <p class="text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
